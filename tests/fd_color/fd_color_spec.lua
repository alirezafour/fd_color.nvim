require("plenary.reload").reload_module("fd_color", true)
local fd_color = require("fd_color")
local default = fd_color.config
...
describe("tests", function()
  it("works with default values", function()
    fd_color.setup()
    assert.are.same(fd_color.config, default)
  end)
  ...
  it("works with config overrides", function()
    ...
    fd_color.setup({ undercurl = false, underline = false })
    assert.are.same(fd_color.config, expected)
  end)
  ...
  it("does not set terminal colors when terminal_colors is false", function()
    clear_term_colors()
    fd_color.setup({ terminal_colors = false })
    fd_color.load()
  ...
  it("sets terminal colors when terminal_colors is true", function()
    clear_term_colors()
    fd_color.setup({ terminal_colors = true })
    fd_color.load()

    -- dark bg
    local colors = require("fd_color").palette
